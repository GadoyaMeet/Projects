...
# Dlib detector and models
detector = dlib.get_frontal_face_detector()
predictor = dlib.shape_predictor('data/data_dlib/shape_predictor_68_face_landmarks.dat')
face_reco_model = dlib.face_recognition_model_v1("data/data_dlib/dlib_face_recognition_resnet_model_v1.dat")

# Create a connection and attendance table
conn = sqlite3.connect("attendance.db")
cursor = conn.cursor()
table_name = "attendance"
create_table_sql = f"""
...
class Face_Recognizer:
    ...
    def get_face_database(self):
        ...
        if os.path.exists(path_features_known_csv):
            if os.path.getsize(path_features_known_csv) == 0:
                logging.warning("'features_all.csv' exists but is empty!")
                logging.warning("Run 'get_faces_from_camera.py' and 'features_extraction_to_csv.py' to populate it.")
                return 0
            csv_rd = pd.read_csv(path_features_known_csv, header=None)
            for i in range(csv_rd.shape[0]):
                features_someone_arr = []
                ...
                for j in range(1, 129):
                    val = csv_rd.iloc[i][j]
                    features_someone_arr.append(float(val) if val != '' else 0.0)
                self.face_features_known_list.append(features_someone_arr)
            logging.info("Faces loaded from Database: %d", len(self.face_features_known_list))
            return 1
        else:
            logging.warning("'features_all.csv' not found!")
            logging.warning("Please run 'get_faces_from_camera.py' and 'features_extraction_to_csv.py' first.")
            return 0
    ...
    def centroid_tracker(self):
        for i in range(len(self.current_frame_face_centroid_list)):
            e_distance_list = []
            for j in range(len(self.last_frame_face_centroid_list)):
                dist = self.return_euclidean_distance(
                    self.current_frame_face_centroid_list[i],
                    self.last_frame_face_centroid_list[j])
                e_distance_list.append(dist)

            last_frame_num = e_distance_list.index(min(e_distance_list))
            self.current_frame_face_name_list[i] = self.last_frame_face_name_list[last_frame_num]

    def draw_note(self, img_rd):
        cv2.putText(img_rd, "Face Recognizer with Deep Learning", (20, 40), self.font, 1, (255, 255, 255), 1)
        cv2.putText(img_rd, f"Frame:  {self.frame_cnt}", (20, 100), self.font, 0.8, (0, 255, 0), 1)
        cv2.putText(img_rd, f"FPS:    {round(self.fps, 2)}", (20, 130), self.font, 0.8, (0, 255, 0), 1)
        ...
        for i, name in enumerate(self.current_frame_face_name_list):
            cv2.putText(img_rd, "Face_" + str(i + 1),
                        tuple(map(int, self.current_frame_face_centroid_list[i])),
                        self.font, 0.8, (255, 190, 0), 1)
    ...
    def attendance(self, name):
        ...
        if existing:
            print(f"{name} already marked present for {current_date}")
        else:
            current_time = datetime.datetime.now().strftime('%H:%M:%S')
            cursor.execute("INSERT INTO attendance (name, time, date) VALUES (?, ?, ?)",
                           (name, current_time, current_date))
            conn.commit()
            print(f"{name} marked as present at {current_time} on {current_date}")
        conn.close()
        ...
        while stream.isOpened():
            ...
            if self.current_frame_face_cnt == self.last_frame_face_cnt and self.reclassify_interval_cnt != self.reclassify_interval:
                ...
                if self.current_frame_face_cnt != 0:
                    self.current_frame_face_position_list = []
                    for k, d in enumerate(faces):
                        self.current_frame_face_position_list.append((d.left(), d.bottom() + (d.bottom() - d.top()) // 4))
                        self.current_frame_face_centroid_list.append([(d.left() + d.right()) / 2, (d.top() + d.bottom()) / 2])
                        img_rd = cv2.rectangle(img_rd, (d.left(), d.top()), (d.right(), d.bottom()), (255, 255, 255), 2)
                ...
                if self.current_frame_face_cnt == 0:
                    ...
                    for k in range(len(faces)):
                        self.current_frame_face_centroid_list.append([(faces[k].left() + faces[k].right()) / 2,
                                                                       (faces[k].top() + faces[k].bottom()) / 2])
                        self.current_frame_face_X_e_distance_list = []
                        self.current_frame_face_position_list.append((faces[k].left(), faces[k].bottom() + (faces[k].bottom() - faces[k].top()) // 4))

                        for i in range(len(self.face_features_known_list)):
                            if float(self.face_features_known_list[i][0]) != 0.0:
                                dist = self.return_euclidean_distance(
                                    self.current_frame_face_feature_list[k],
                                    self.face_features_known_list[i])
                                self.current_frame_face_X_e_distance_list.append(dist)
                            else:
                                self.current_frame_face_X_e_distance_list.append(999999999)

                        min_dist = min(self.current_frame_face_X_e_distance_list)
                        similar_person_num = self.current_frame_face_X_e_distance_list.index(min_dist)
